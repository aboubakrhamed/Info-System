...
function executePrint() {
    ...
    const printContent = `<!DOCTYPE html><html lang="${lang}" dir="${dir}"><head><meta charset="UTF-8"><base href="${baseUrl}"><title>${t.appTitle} - Print View</title><script src="https://cdn.tailwindcss.com"></script><link href="https://fonts.googleapis.com/css2?family=Inter:wght@400;500;600;700;900&family=Cairo:wght@400;600;700;900&display=swap" rel="stylesheet"><link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/6.4.0/css/all.min.css"><script src="https://unpkg.com/lucide@latest"></script><script>tailwind.config = {theme: {extend: {colors: { fjGold: '#C5A059', fjNavy: '#0B1120' },fontFamily: { sans: ['Inter', 'Cairo', 'sans-serif'] }}}}</script><style>body { -webkit-print-color-adjust: exact; print-color-adjust: exact; background-color: #fff; } @media print { @page { margin: 15mm; } .no-print { display: none !important; } }</style></head><body class="font-sans text-slate-800 p-8"><div class="bg-fjNavy p-6 rounded-xl mb-6 flex items-center justify-between"><div class="flex items-center gap-3"><i class="fa-solid fa-graduation-cap text-fjGold text-4xl drop-shadow-[0_2px_2px_rgba(0,0,0,0.3)]"></i><div class="flex flex-col justify-center mt-0.5"><h1 class="text-3xl font-black text-fjGold uppercase tracking-tight m-0 leading-none">FUTURE JOURNEY</h1><p class="text-[10px] text-fjGold/90 tracking-[0.15em] font-medium mt-1 uppercase w-full">Your path to your passion</p></div></div><div class="text-sm font-bold text-fjGold bg-fjGold/10 border border-fjGold/30 px-3 py-1 rounded">${filteredData.length} ${t.programsCount}</div></div><div class="border border-slate-200 rounded-xl overflow-hidden shadow-sm"><div class="grid grid-cols-12 bg-slate-100 border-b border-slate-200 py-3 px-5 text-xs font-bold text-slate-600 uppercase tracking-wider"><div class="col-span-3">${t.colUniversity}</div><div class="col-span-4">${t.colProgram}</div><div class="col-span-3">${t.colInfo}</div><div class="col-span-2">${t.colAddress}</div></div><div class="divide-y divide-slate-100 bg-white">${rowsHtml}</div></div><script>setTimeout(() => { lucide.createIcons(); setTimeout(() => { window.print(); }, 800); }, 500);</script></body></html>`;
        ...
        doc.autoTable({
            ...
            didDrawPage: (data) => {
                ...
                if (data.pageNumber === 1) {
                    ...
                    doc.setFontSize(24);
                    doc.text("FUTURE JOURNEY", textStartX, 23);
                    doc.setFont("helvetica", "normal");
                    doc.setFontSize(9);
                    doc.text("YOUR PATH TO YOUR PASSION", textStartX, 30);
                }
            },
            didDrawCell: (data) => {
